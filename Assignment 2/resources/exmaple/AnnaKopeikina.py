...
"""Name of input and outputs files"""
...
def calculate_rating(population):
    for chromosome in population:
        ...
        for i in range(chromosome.size):
            if final_genom_chord.has_in_consonant_chords(
                    chromosome.genes_pool[i]):  # if this gene is in consonant fitting chords, we reward it
                chromosome.rating -= 0.5
                if (song_notes[i] is None):
                    chromosome.rating -= 0.5
                    continue
                if (not final_genom_chord.has_note_in_consonant_chords(song_notes[i]) or chromosome.genes_pool[
                    i].has_note(song_notes[i])):  # if this genes chord is fitting to the music, we reward it
                    chromosome.rating -= 0.5
...
while True:
    ...
    mutate(population, size // 2,
           1)  # We mutate always only one gene, as it will allow quicker evolution for almost perfect chords

"""Now we append track using the best generated accompanement and write it into the new outputs file"""
